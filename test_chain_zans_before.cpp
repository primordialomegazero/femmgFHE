...
int main() {
    cout << "\n";
    cout << "╔══════════════════════════════════════════════╗\n";
    cout << "║  CHAINED SQUARING — ZANS BEFORE MULTIPLY    ║\n";
    cout << "║  Apply ZANS to boost noise budget first      ║\n";
    cout << "╚══════════════════════════════════════════════╝\n";
    cout << "\n";
    ...
    cout << "Step | Expected | Decrypted | Noise   | ZANS Before | Status\n";
    cout << "-----|----------|-----------|---------|-------------|-------\n";
    ...
    for (int step = 0; step < 12; step++) {
        ...
        cout << "  " << setw(3) << step << " | "
             ...
             << setw(7) << noise << " bits"
             << (noise > 0 ? " ✅" : " ❌") << " | "
             << setw(11) << (zans_counter > 0 ? "ZANS" : "none")
             << " | "
        ...
        // Apply ZANS BEFORE squaring (every 2 steps)
        if (step % 2 == 0 && step > 0) {
            int before = decryptor.invariant_noise_budget(ct);
            cout << "  → ZANS BEFORE (10 iters): " << before << " bits";
            apply_zans(ct, evaluator, encryptor, context, 10);
