...
struct CompetitiveFractal {
    ...
    CompetitiveFractal(int n, const std::vector<std::vector<int>>& cls)
    ...
    void apply_competitive_tension() {
        for (const auto& cl : clauses) {
            ...
            for (int lit : cl) {
                int var = abs(lit) - 1;
                double val = (lit > 0) ? observer[var] : observed[var];
                if (val > second_best && lit != best_lit) {
                    second_best = val;
                }
            }

            double gap = best_val - second_best;
            double push = (1.0 / PHI) * (1.0 - best_val);
            ...
            for (int lit : cl) {
                ...
                if (lit == best_lit) {
                    ...
                    double weaken = push * 0.5;
                    if (lit > 0) {
                        observer[var] -= weaken * observer[var];
                        if (observer[var] < 0.01) observer[var] = 0.01;
                    } else {
                        observed[var] -= weaken * observed[var];
                        if (observed[var] < 0.01) observed[var] = 0.01;
                    }
                }
    ...
    bool detect() {
        ...
        for (int iter = 0; iter < n_vars * 3; iter++) {
            fractal_diffusion();
...
int main() {
    std::cout << "\n";
    std::cout << "  COMPETITIVE FRACTAL MIRROR\n";
    ...
    int correct = 0;
    for (auto& t : tests) {
        CompetitiveFractal detector(t.vars, t.clauses);
