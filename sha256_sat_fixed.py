#!/usr/bin/env python3
"""
🔐 SHA-256 SAT CIRCUIT — FIXED (Gamma0/Gamma1 for message schedule)
"""
...
class SHA256SatCircuit:
    ...
    # ═══════════════════════════════════════
    # COMPRESSION FUNCTIONS (for rounds)
    # ═══════════════════════════════════════
    ...
    # ═══════════════════════════════════════
    # MESSAGE SCHEDULE FUNCTIONS (for W[16..63])
    # ═══════════════════════════════════════
    def GAMMA0_word(self, X):
        """γ0(x) = ROTR(x,7) ^ ROTR(x,18) ^ SHR(x,3)"""
    ...
    def GAMMA1_word(self, X):
        """γ1(x) = ROTR(x,17) ^ ROTR(x,19) ^ SHR(x,10)"""
    ...
    def compress(self, msg_bits, rounds=64):
        """FULL SHA-256 compression with CORRECT message schedule"""
        ...
        # Message schedule: W[16..63] using GAMMA0/GAMMA1
        for t in range(16, 64):
            g0 = self.GAMMA0_word(W[t-15])   # FIXED: use Gamma, not Sigma!
            g1 = self.GAMMA1_word(W[t-2])    # FIXED: use Gamma, not Sigma!
        ...
        final = []
        state_vars = [a, b, c, d, e, f, g, h]
        for i in range(8):
            final.append(self.ADD_word(state_vars[i], state[i]))
...
if __name__ == '__main__':
    print("╔══════════════════════════════════════════════════════════════╗")
    print("║  🔐 SHA-256 SAT CIRCUIT — FIXED (Gamma0/Gamma1)             ║")
    print("╚══════════════════════════════════════════════════════════════╝\n")
    ...
    try:
        ...
        # Force first 64 bits (2 words) to match
        ...
        if result:
            ...
            print(f"  🎉 FULL SHA-256 CIRCUIT WORKS!")
            print(f"  Estimated for Bitcoin attack: ~{(sha.next_var - 1) * 1.3:.0f} vars (with RIPEMD-160)")
        else:
        ...
        print(f"\n╔══════════════════════════════════════════════════════════════╗")
        print(f"║  🔐 SHA-256 SAT CIRCUIT — FIXED & READY                     ║")
        print(f"╚══════════════════════════════════════════════════════════════╝")
