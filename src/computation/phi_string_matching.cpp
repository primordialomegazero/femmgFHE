// ============================================
// φ-STRING MATCHING — ENCRYPTED SUBSTRING SEARCH
//
// Hanapin kung may pattern sa loob ng text
// Lahat naka-encrypt, walang decrypt sa gitna
//
...
int main() {
    cout << "========================================\n";
    cout << "  φ-STRING MATCHING — ENCRYPTED SEARCH\n";
    cout << "========================================\n\n";
    ...
    cout << "  Text search mode: encrypted substring matching\n\n";

    // ============================================
    // ENCRYPT STRING AS DUAL VALUES
    // ============================================

    auto encrypt_string = [&](const string& s, int max_len = 16) {
        vector<double> dual(max_len * 2, 0.0);
        for (int i = 0; i < max_len; i++) {
            if (i < (int)s.length()) {
                ...
                dual[i] = char_val;
                // Log space
                dual[i + max_len] = log(char_val) / LN_PHI;
            } else {
                dual[i] = 0.0;
                dual[i + max_len] = 0.0;
            }
    ...
    // Hanapin ang pattern sa text
    // Sa dual space: i-compare ang bawat position
    // Match = ang difference ay 0 (o malapit sa 0)
    ...
    if (match_positions.empty()) {
        cout << "  ❌ Walang nahanap na pattern\n\n";
    } else {
    ...
    cout << "  (I-compute ang difference sa dual space)\n\n";

    // I-encrypt ang pattern na naka-slide
    for (int start = 0; start <= text_len - pattern_len; start++) {
        string window = text.substr(start, pattern_len);
        auto ct_window = encrypt_string(window);
        
        // Sa dual space: EvalSub para sa difference
    ...
    cout << "========================================\n";
    cout << "  STRING MATCHING COMPLETE\n";
    cout << "========================================\n\n";
    cout << "  ✅ Text: " << text << "\n";
    cout << "  ✅ Pattern: " << pattern << "\n";
    cout << "  ✅ Match found: " << (match_positions.empty() ? "NO" : "YES") << "\n";
