...
namespace GoldenUltimate {

constexpr double PHI = 1.6180339887498948482;
constexpr double PSI = -0.6180339887498948482;

class GoldenUltimateSolver {
public:
    struct Result {
        bool satisfiable;
...
private:
    ...
    // Golden ratio clause satisfaction
    ...
    // Unit propagation with golden ratio learning
        ...
        while (changed) {
            ...
            for (auto& clause : clauses) {
                ...
                if (unassigned == 0) {
                    ...
                    // Golden ratio conflict learning
                    clause.activity *= PHI;
                    return false;
    ...
    static int select_variable(const std::vector<Clause>& clauses,
        ...
        for (int v = 0; v < num_vars; v++) {
            ...
            if (total > 0) {
                // Golden ratio balance: optimal para sa branching
    ...
    static bool dpll_recursive(std::vector<Clause>& clauses,
        ...
        // Golden ratio polarity preference
        int pos_count = 0, neg_count = 0;
        for (const auto& clause : clauses) {
            for (int lit : clause.literals) {
                if (abs(lit) - 1 == var) {
        ...
        // Golden ratio decision: prefer balanced polarity
        bool try_true = (pos_count * PSI + neg_count * PHI) >= 0;
        
        // Save state para sa backtracking
        std::vector<int> assignment_snapshot = assignment;
        int num_assigned_snapshot = num_assigned;
        ...
        // Restore at try other polarity
        assignment = assignment_snapshot;
        num_assigned = num_assigned_snapshot;
...
public:
    static Result solve(const std::vector<std::vector<int>>& clauses_input, int num_vars) {
        ...
        std::vector<double> var_activity(num_vars, 1.0);
        int num_assigned = 0;
        ...
        int satisfied = 0;
        for (const auto& clause : clauses) {
            if (clause_satisfied(clause, assignment)) satisfied++;
        }
