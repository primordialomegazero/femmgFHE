// ============================================
// φ-EPSILON SERIES
// I-verify ang infinite series para sa ε_n
// ε_n = -φ^(-2n) + φ^(-4n)/2 - φ^(-6n)/3 + ...
// ============================================
...
int main() {
    ...
    cout << "=== φ-EPSILON SERIES ===\n\n";
    cout << "  ε_n = -φ^(-2n) + φ^(-4n)/2 - φ^(-6n)/3 + ...\n\n";
    ...
    for (int n = 3; n <= 20; n++) {
        ...
        // Compute series: -φ^(-2n) + φ^(-4n)/2 - φ^(-6n)/3
        double eps_series = 0;
        for (int k = 1; k <= 5; k++) {
            double term = pow(-1.0, k) * pow(PHI, -2.0 * k * n) / k;
            eps_series += term;
        }
