// ============================================
// φ-ADAPTIVE DECODE — HALF-SPLIT THRESHOLD
//
// Simple: mod_phi < φ/2 → 1, else → 0
// Walang complex table — simple half-split!
//
...
int main() {
    ...
    cout << "  Half-split: mod_phi < φ/2 → 1, else → 0\n\n";
    ...
    // SIMPLE HALF-SPLIT DECODE
    auto half_split_decode = [&](double val) {
    ...
    for (int A : {0, 1}) {
        for (int B : {0, 1}) {
            for (int Cin : {0, 1}) {
                ...
                int decoded = half_split_decode(avg);
