// ============================================
// φ-BEATTY FIX2 — TAMANG MASKS
// Addition: Slot 0 at 1 lang
// Multiplication: Slot 2 lang
// ============================================
...
int main() {
    ...
    // Normal-space mask: Slot 0 at 1 lang
    auto encrypt_normal = [&](double F) {
        int q = (int)floor(F / PHI);
        int grupo = q % 2;
        vector<double> v(4, 0.0);
        v[0] = fmod(F, PHI);
        v[1] = grupo;
        v[2] = 0;  // WALANG log delta
        v[3] = 0;
        Plaintext pt = cc->MakeCKKSPackedPlaintext(v);
        return cc->Encrypt(keyPair.publicKey, pt);
    };
    ...
    cout << "========================================\n";
    cout << "  φ-BEATTY FIX2 — TAMANG MASKS\n";
    cout << "========================================\n\n";

    int N = 100;

    auto ct_state = encrypt_full(100.0);
    double expected = 100.0;

    auto ct_add = encrypt_normal(7.0);
    auto ct_sub = encrypt_normal(-3.0);
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (op == 0) {
            ...
            ct_state = cc->EvalAdd(ct_state, ct_sub);  // negative na
            expected -= 3.0;
        } else if (op == 2) {
