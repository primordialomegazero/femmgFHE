// PHI-OMEGA-ZERO: FIBONACCI DECOMPOSITION MATRIX OBFUSCATION
// Zeckendorf: break scalar into Fibonacci numbers
// ×1000000 = ×832040 + ×121393 + ×46368 + ... (~15 terms instead of 1M!)
// "FIBONACCI DECOMPOSES. THE ADDITIONS COLLAPSE."
// "I AM THAT I AM"

#include <openfhe.h>
#include <iostream>
#include <iomanip>
#include <vector>
#include <random>
#include <chrono>
#include <ctime>
#include <sstream>

using namespace lbcrypto;
using namespace std;
using namespace std::chrono;

string ts() { auto now=system_clock::now(); auto t=system_clock::to_time_t(now); stringstream ss; ss<<put_time(localtime(&t),"%Y-%m-%d %H:%M:%S"); return ss.str(); }

class FibDecompIO {
    CryptoContext<DCRTPoly> cc; KeyPair<DCRTPoly> keys; Ciphertext<DCRTPoly> anchor0;
    int64_t MOD, half_mod; mt19937 rng; static const int N=3;
    using Matrix=vector<vector<int64_t>>;
    vector<int64_t> fib;

    int64_t mod(int64_t v){return((v%MOD)+MOD)%MOD;}
    int64_t minv(int64_t a){int64_t t=0,nt=1,r=MOD,nr=mod(a);while(nr){int64_t q=r/nr;int64_t tmp=t;t=nt;nt=tmp-q*nt;tmp=r;r=nr;nr=tmp-q*nr;}return(r>1)?1:mod(t);}
    Ciphertext<DCRTPoly> enc(int64_t v){return cc->Encrypt(keys.publicKey,cc->MakePackedPlaintext(vector<int64_t>{mod(v)}));}
    int64_t dec(const Ciphertext<DCRTPoly>& ct){Plaintext pt;cc->Decrypt(keys.secretKey,ct,&pt);return pt->GetPackedValue()[0];}
    Matrix I(){Matrix m(N,vector<int64_t>(N,0));for(int i=0;i<N;i++)m[i][i]=1;return m;}
    Matrix mmul(const Matrix&A,const Matrix&B){Matrix C(N,vector<int64_t>(N,0));for(int i=0;i<N;i++)for(int j=0;j<N;j++)for(int k=0;k<N;k++)C[i][j]=mod(C[i][j]+mod(A[i][k]*B[k][j]));return C;}
    Matrix dmat(){uniform_int_distribution<int64_t> d(2,MOD-1);Matrix D(N,vector<int64_t>(N,0));for(int i=0;i<N;i++)D[i][i]=d(rng);return D;}
    Matrix dinv(const Matrix&D){Matrix R(N,vector<int64_t>(N,0));for(int i=0;i<N;i++)R[i][i]=minv(D[i][i]);return R;}

    void gen_fib(){fib={1,2};while(fib.back()<MOD){fib.push_back(fib[fib.size()-1]+fib[fib.size()-2]);}}

    // Zeckendorf: decompose n into Fibonacci numbers
    vector<int64_t> zeckendorf(int64_t n){
        vector<int64_t> terms;
        for(int i=fib.size()-1;i>=0 && n>0;i--){
            if(fib[i]<=n){terms.push_back(fib[i]);n-=fib[i];}
        }
        return terms;
    }

    // Multiply state by scalar using Fibonacci decomposition + True Divine per step
    Ciphertext<DCRTPoly> fib_multiply(const Ciphertext<DCRTPoly>& state, int64_t scalar){
        if(scalar==0)return enc(0);
        auto terms=zeckendorf(scalar);
        auto result=enc(0);
        for(auto t:terms){
            // Add state t times (but t is a Fibonacci number, relatively small!)
            // Actually: multiply state × t using repeated addition of 'state'
            auto term=state;
            for(int64_t i=1;i<t;i++){
                // Pinky Swear
                auto M=enc(half_mod);auto s=cc->EvalAdd(term,M);s=cc->EvalAdd(s,anchor0);
                auto b=cc->EvalSub(s,M);b=cc->EvalAdd(b,anchor0);auto ov=cc->EvalSub(term,b);ov=cc->EvalAdd(ov,anchor0);
                // Add
                term=cc->EvalAdd(term,state);
                // ZANS
                term=cc->EvalAdd(term,anchor0);term=cc->EvalAdd(term,anchor0);term=cc->EvalAdd(term,anchor0);
                // Divine
                auto divine=cc->EvalMult(ov,anchor0);divine=cc->EvalAdd(divine,anchor0);
                term=cc->EvalAdd(term,divine);term=cc->EvalAdd(term,divine);
            }
            result=cc->EvalAdd(result,term);
            result=cc->EvalAdd(result,anchor0);
        }
        return result;
    }

    Matrix weight_matrix(int64_t w){return {{1,w,mod(w*w)},{0,1,mod(2*w)},{0,0,1}};}

public:
    FibDecompIO():rng(time(nullptr)){MOD=1073643521;half_mod=MOD/2;gen_fib();CCParams<CryptoContextBFVRNS> p;p.SetMultiplicativeDepth(10);p.SetPlaintextModulus(MOD);p.SetRingDim(2048);p.SetSecurityLevel(HEStd_NotSet);cc=GenCryptoContext(p);cc->Enable(PKE);cc->Enable(KEYSWITCH);cc->Enable(LEVELEDSHE);cc->Enable(ADVANCEDSHE);keys=cc->KeyGen();cc->EvalMultKeyGen(keys.secretKey);anchor0=enc(0);}

    int64_t evaluate(int64_t x, int bits){
        vector<Ciphertext<DCRTPoly>> state(N);
        state[0]=enc(1);state[1]=enc(0);state[2]=enc(0);
        for(int b=0;b<bits;b++)if(x&(1LL<<b)){
            int64_t w=1LL<<b;
            Matrix M_plain=weight_matrix(w);
            Matrix R=dmat(),Ri=dinv(R);
            Matrix M_obf=mmul(R,mmul(M_plain,Ri));
            vector<Ciphertext<DCRTPoly>> ns(N);
            for(int c=0;c<N;c++){auto acc=enc(0);for(int k=0;k<N;k++){auto term=fib_multiply(state[k],M_obf[k][c]);acc=cc->EvalAdd(acc,term);acc=cc->EvalAdd(acc,anchor0);}ns[c]=acc;}
            state=ns;
        }
        auto two_x=fib_multiply(state[1],2);
        auto out=cc->EvalAdd(state[2],cc->EvalAdd(two_x,state[0]));
        out=cc->EvalAdd(out,anchor0);
        return dec(out);
    }

    void demo(){
        cout<<"\n  ╔══════════════════════════════════════════════════════╗\n";
        cout<<  "  ║   FIBONACCI DECOMPOSITION MATRIX OBFUSCATION         ║\n";
        cout<<  "  ║   Date: "<<ts()<<"                         ║\n";
        cout<<  "  ╚══════════════════════════════════════════════════════╝\n\n";
        cout<<"  Fibonacci numbers: "<<fib.size()<<" up to "<<fib.back()<<"\n";
        cout<<"  Zeckendorf: any scalar → ~log_φ(n) terms\n\n";
        cout<<"  4-BIT TEST:\n  "<<string(45,'-')<<"\n  "<<setw(8)<<"x"<<setw(15)<<"FHE"<<setw(15)<<"Expected\n  "<<string(45,'-')<<"\n";
        bool ok=true;
        for(int x:{0,1,2,3,5}){
            int64_t r=evaluate(x,4);
            int64_t e=(x+1)*(x+1);
            if(r!=e)ok=false;
            cout<<"  "<<setw(8)<<x<<setw(15)<<r<<setw(15)<<e<<(r==e?" OK":" FAIL")<<"\n";
        }
        cout<<"  "<<string(45,'-')<<"\n  4-bit: "<<(ok?"ALL CORRECT":"ERRORS")<<"\n\n  I AM THAT I AM\n\n";
    }
};

int main(){FibDecompIO io;io.demo();return 0;}
