// PHI-OMEGA-ZERO: PURE CT×CT iO — 16-BIT FINAL
// Ring dim 16384, 30-bit modulus, True Divine loop
// f(x)=x×x + 2×x + 1 — 2 CT×CT, all encrypted
// "RING DIM 16384. THE HOLY GRAIL SCALES."
// "I AM THAT I AM"
...
public:
    FinalIO(){MOD=1073643521;half_mod=MOD/2;CCParams<CryptoContextBFVRNS> p;p.SetMultiplicativeDepth(10);p.SetPlaintextModulus(MOD);p.SetRingDim(16384);p.SetSecurityLevel(HEStd_NotSet);cc=GenCryptoContext(p);cc->Enable(PKE);cc->Enable(KEYSWITCH);cc->Enable(LEVELEDSHE);cc->Enable(ADVANCEDSHE);keys=cc->KeyGen();cc->EvalMultKeyGen(keys.secretKey);anchor0=enc(0);}
    ...
    void demo(){
        cout<<"\n  ╔══════════════════════════════════════════════════════╗\n";
        cout<<  "  ║   PURE CT×CT iO — 16-BIT FINAL                        ║\n";
        cout<<  "  ║   Ring Dim 16384 | 2 CT×CT | True Divine              ║\n";
        cout<<  "  ║   Date: "<<ts()<<"                         ║\n";
        cout<<  "  ╚══════════════════════════════════════════════════════╝\n\n";
        
        cout<<"  16-BIT FULL TEST:\n  "<<string(55,'-')<<"\n  "<<setw(10)<<"x"<<setw(20)<<"FHE"<<setw(20)<<"Expected\n  "<<string(55,'-')<<"\n";
        bool ok=true;
...
int main(){FinalIO io;io.demo();return 0;}
