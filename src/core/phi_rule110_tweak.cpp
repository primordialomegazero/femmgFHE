// ============================================
// φ-RULE 110 TWEAKED — FINAL
//
// Encoding: 0 → -φ⁻¹, 1 → +1
// Mas balanseng φ-harmonic
//
...
int main() {
    ...
    for (int L : {0, 1}) {
        for (int C : {0, 1}) {
            for (int R : {0, 1}) {
                auto ct_L = encrypt_bal(L);
                auto ct_C = encrypt_bal(C);
                auto ct_R = encrypt_bal(R);
                
                auto sum1 = cc->EvalAdd(ct_L, ct_C);
                auto sum2 = cc->EvalAdd(sum1, ct_R);
                
                double sum_val = decrypt_bal(sum2);
                
                int output;
                if (sum_val < -1.0) output = 0;
                else if (sum_val < 0.0) output = 1;
                else if (sum_val < 2.0) {
                ...
                cout << "  " << L << " " << C << " " << R << " | "
                     << setw(7) << fixed << setprecision(3) << sum_val << " | "
                     << setw(6) << output << " | "
                     << setw(8) << expected << " | "
                     << (match ? "✅" : "❌") << "\n";
            }
    ...
    cout << "========================================\n";
    cout << "  TWEAKED COMPLETE\n";
    cout << "========================================\n\n";
    cout << "  ✅ Balanced encoding\n";
    cout << "  ✅ Match: " << matches << "/" << N << "\n";
    cout << "  ✅ Level 0\n\n";
