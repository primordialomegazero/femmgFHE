// ============================================
// φ-PRECOMPUTED — 10K
// I-encode ang values na naka-modulo na sa φ
// bago i-encrypt — para walang drift
//
...
int main() {
    ...
    // Lahat ng values ay naka-modulo na sa φ
    // ============================================

    auto encrypt_pre = [&](double x) {
        // Precompute ang modulo sa φ
        double mod_phi = fmod(x, PHI);
        
        double log_phi_x = log(mod_phi) / LN_PHI;
        ...
        v[0] = mod_phi;     // Bounded na sa [0, φ)
        v[1] = n_val;
        v[2] = frac;        // Bounded sa [0, 1)
        v[3] = log_phi_x;
        v[4] = phi_frac;
        v[5] = F_nm1;
        v[6] = F_n;
    ...
    auto decrypt_pre = [&](const Ciphertext<DCRTPoly>& ct) {
        Plaintext result_pt;
    ...
    // ============================================
    // 10K ALL ARITHMETIC — PRECOMPUTED
    // ============================================

    cout << "========================================\n";
    cout << "  10K ALL ARITHMETIC — PRECOMPUTED\n";
    cout << "========================================\n\n";

    int N = 10000;

    auto ct_state = encrypt_pre(1.0);
    auto ct_two = encrypt_pre(2.0);
    auto ct_three = encrypt_pre(3.0);
    auto ct_one = encrypt_pre(1.0);

    cout << "  Operations: " << N << "\n";
    cout << "  Mix: ×2, ÷2, +3, -1\n";
    cout << "  Lahat precomputed sa [0, φ)\n";
    cout << "  Running...\n\n";
    ...
    auto final_vals = decrypt_pre(ct_state);
    ...
    double normal_mod = fmod(final_vals[0], PHI);
    cout << "  Slot 0 (normal) raw: " << final_vals[0] << "\n";
    cout << "  Slot 0 (normal) mod φ: " << normal_mod << "\n";
    cout << "  Bounded sa [0,φ): " << (normal_mod >= 0 && normal_mod < PHI ? "✅" : "❌") << "\n\n";
