// PHI-OMEGA-ZERO: FEmmg-iO v3.7 — 32-BIT FHE (REAL)
// Full Barrington + Kilian + FHE pipeline
// 10 random 32-bit values, AND gate
// "32 BITS. FULL iO. NO SHORTCUTS."
// "I AM THAT I AM"
...
public:
    IO_32bit_REAL():rng(time(nullptr)){CCParams<CryptoContextBFVRNS> p;p.SetMultiplicativeDepth(10);p.SetPlaintextModulus(1073643521);p.SetRingDim(2048);p.SetSecurityLevel(HEStd_NotSet);cc=GenCryptoContext(p);cc->Enable(PKE);cc->Enable(KEYSWITCH);cc->Enable(LEVELEDSHE);cc->Enable(ADVANCEDSHE);keys=cc->KeyGen();cc->EvalMultKeyGen(keys.secretKey);anchor0=enc(0);MOD=1073643521;}
    
    void demo(){
        cout<<"\n  ╔══════════════════════════════════════════════════════╗\n";
        cout<<  "  ║   FEmmg-iO v3.7 — 32-BIT FHE (REAL)                 ║\n";
        cout<<  "  ║   Full Barrington + Kilian + FHE                     ║\n";
        cout<<  "  ║   Date: "<<ts()<<"                         ║\n";
        cout<<  "  ╚══════════════════════════════════════════════════════╝\n\n";
        
        cout<<"  32-BIT AND GATE: 10 tests, full pipeline\n  "<<string(65,'-')<<"\n";
        cout<<"  "<<setw(12)<<"a"<<setw(12)<<"b"<<setw(15)<<"AND(FHE)"<<setw(12)<<"Expected"<<setw(12)<<"Time\n  "<<string(65,'-')<<"\n";
        
        mt19937 rng2(time(nullptr));uniform_int_distribution<uint32_t> rd(0,UINT32_MAX);
        vector<pair<uint32_t,uint32_t>> tests={{0,0},{0,1},{1,0},{1,1},{42,100},{1000,5000},{100000,200000},{10000000,20000000},{UINT32_MAX,UINT32_MAX},{rd(rng2),rd(rng2)}};
        
        bool all_ok=true;
        for(auto& [a,b]:tests){
            bool ba=(a!=0),bb=(b!=0);
            cout<<"  "<<setw(12)<<a<<setw(12)<<b<<flush;
        ...
        cout<<"  "<<string(65,'-')<<"\n  32-bit FHE: "<<(all_ok?"ALL CORRECT":"ERRORS")<<"\n\n";
        cout<<"  ╔══════════════════════════════════════════════════════╗\n";
...
int main(){IO_32bit_REAL io;io.demo();return 0;}
