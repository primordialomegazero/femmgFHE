// ============================================
// φ-ZERO ADAPTIVE — ZERO DETECTION + HALF-SPLIT
//
// Zero detection: mod_phi < 0.1 || mod_phi > φ-0.1 → 0
// Otherwise: mod_phi < φ/2 → 1, else → 0
//
...
int main() {
    ...
    const double ZERO_THRESHOLD = 0.1;

    cout << "  ✅ CKKS initialized (depth 1, modsize 59!)\n";
    cout << "  Zero detection + half-split\n\n";
    ...
    // ZERO DETECTION + HALF-SPLIT DECODE
    auto zero_adaptive_decode = [&](double val) {
        double mod_phi = fmod(val, PHI);
        
        // Zero detection: malapit sa 0 o φ
        if (mod_phi < ZERO_THRESHOLD || mod_phi > PHI - ZERO_THRESHOLD) return 0;
        
        // Otherwise: half-split
