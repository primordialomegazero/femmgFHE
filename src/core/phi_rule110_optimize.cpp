// ============================================
// φ-RULE 110 OPTIMIZE — V2 FINE-TUNE
//
// I-optimize ang Multi-Log V2 (12/16):
// 1. ScalingModSize variations
// 2. Symmetric vs asymmetric encoding
// 3. Extended φ-power sweep
//
...
int main() {
    ...
    cout << "  TESTING: Symmetric encoding (0→-base, 1→+base)\n";
    ...
    // Extended sweep
    vector<double> L_powers = {-3.0, -2.5, -2.0, -1.5, -1.0};
    vector<double> R_powers = {0.5, 0.75, 1.0, 1.25, 1.5, 2.0};
    vector<int> modsizes = {50, 55, 59};
        ...
        for (double L_pow : L_powers) {
            for (double R_pow : R_powers) {
                ...
                if (matches > best_match) {
                    ...
                    best_L = L_pow;
                    best_R = R_pow;
                    best_modsize = modsize;
                    cout << "  " << setw(7) << fixed << setprecision(2) << L_pow << " | "
                         << setw(7) << R_pow << " | "
                         << setw(5) << matches << "/16 ⭐ (mod " << modsize << ")\n";
                }
            }
        }
    }
    ...
    cout << " → " << best_match << "/16 (mod " << best_modsize << ")\n\n";
