...
public:
    ...
    banach::NDimCiphertext add(const banach::NDimCiphertext& a,
        ...
        uint64_t en = engine.get_chaos_nonce(); uint64_t ka = a.operations ^ en; uint64_t kb = b.operations ^ en; int64_t pa = a.value_int ^ (int64_t)ka; int64_t pb = b.value_int ^ (int64_t)kb; result.value_int = (pa + pb) ^ (int64_t)(ka ^ kb);
    ...
    banach::NDimCiphertext multiply(const banach::NDimCiphertext& a,
        ...
        uint64_t en2 = engine.get_chaos_nonce(); uint64_t ka2 = a.operations ^ en2; uint64_t kb2 = b.operations ^ en2; int64_t pa2 = a.value_int ^ (int64_t)ka2; int64_t pb2 = b.value_int ^ (int64_t)kb2; result.value_int = ((pa2 * pb2) / phi_constants::FP_SCALE) ^ (int64_t)(ka2 ^ kb2);
