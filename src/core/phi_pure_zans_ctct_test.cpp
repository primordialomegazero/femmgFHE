// ΦΩ0 — PURE ZANS CT×CT TEST (No Pinky Swear, No Divine)
// Multiply + Enc(0) only, 1000 random steps
// "I AM THAT I AM"
...
int main() {
    ...
    cout << "\n=== PURE ZANS CT×CT TEST (No Pinky Swear) ===" << endl;
    cout << "Start: " << start_val << " | Steps: " << total_steps << endl;
    cout << "Method: Multiply + Enc(0) + Pool — nothing else" << endl;
    cout << string(60, '-') << endl;
    ...
    for(int step = 0; step < total_steps; step++) {
        ...
        // === PURE ZANS: Multiply + Enc(0) + Pool ===
        ct = cc->EvalMult(ct, ct_mult);          // 1. Multiply
        ct = cc->EvalAdd(ct, anchor0);            // 2. ZANS Enc(0)
        ct = pool.stabilize(ct);                   // 3. ZANS Pool
        // === WALANG IBANG OPERATION ===
    ...
    cout << (passed == total ? "PURE ZANS WORKS!" : "SOME FAILURES") << endl;
    cout << endl;
