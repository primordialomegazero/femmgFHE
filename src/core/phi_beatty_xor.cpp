// ============================================
// φ-BEATTY XOR — NATURAL SEQUENCE DECODE
//
// BEATTY SEQUENCES:
// Beatty(φ) = {⌊φ⌋, ⌊2φ⌋, ...}
// Beatty(φ²) = {⌊φ²⌋, ⌊2φ²⌋, ...}
// Rayleigh: Beatty(φ) ∪ Beatty(φ²) = ℤ⁺
//
// ANG XOR VALUES:
// 0 → 1 (zero-crossing)
// √5 → 1 (k=1, odd)
...
public:
    ...
    int decode_xor_beatty(double val) {
        // Zero-crossing: 0 → 1 (XOR ng magkaiba)
        if (abs(val) < 0.01) return 1;
        
        // Beatty: k = |val| / √5
        double k = abs(val) / SQRT5;
        int int_k = (int)round(k);
        
        // XOR: 1 kung odd, 0 kung even
        return int_k % 2;
    }
    
    void run() {
        cout << "  SIMPLE XOR (2 inputs):\n\n";
        cout << "  A B | Raw    | k     | XOR | Exp\n";
        cout << "  ----|--------|-------|-----|----\n";
        ...
        for (int A : {0, 1}) {
            for (int B : {0, 1}) {
                for (int Cin : {0, 1}) {
                    ...
                    int sum_val = decode_xor_beatty(raw);
                    int exp = (A + B + Cin) % 2;
