...
// 12-bit pack (for V - proven working)
...
// Ternary pack: 2 bits per coef (proven working if normalized)
...
// 10-bit pack: store coef/Q as fraction, not truncated bits
// stored = round(coef * 1023 / 3328). unpack = stored * 3328 / 1023
static void pack10(uint8_t *out, int16_t *in, int len) {
    for(int i=0;i<len/4;i++){
        uint64_t v=0;
        for(int j=0;j<4;j++){
            int32_t c = in[i*4+j];
            int32_t scaled = (c * 1023 + Q/2) / Q;  // round to nearest
            if(scaled>1023) scaled=1023;
            v |= ((uint64_t)scaled) << (j*10);
        }
...
static void unpack10(int16_t *out, uint8_t *in, int len) {
    for(int i=0;i<len/4;i++){
        ...
        for(int j=0;j<4;j++){
            int32_t scaled = (v>>(j*10))&0x3FF;
            out[i*4+j] = (scaled * Q + 512) / 1023;  // round to nearest
        }
    }
}
...
int main() {
    printf("\n  ╔══════════════════════════════════════════════════════╗\n");
    printf(  "  ║   φ-KEM ULTRA v2: Scaled 10-bit packing               ║\n");
    printf(  "  ╚══════════════════════════════════════════════════════╝\n\n");
    ...
    printf("%d/%d passed\n\n",ok,total);
    if(ok==total) printf("  ✅ φ-KEM ULTRA v2 — %dB!\n\n", SKB+PKB+CTB);
