// ΦΩ0 — catchmeifyouKEM v5.0 — NANO (32B CT)
// CT=32B, PK=64B, SK=32B — Total=128B
// 16B encrypted secret + 16B φ-binding (2 iterations)
...
int phi_kem_encaps(uint8_t *ct, uint8_t *ss, const uint8_t *pk) {
    ...
    // φ-chaotic binding (16 bytes = 2 iterations × 8 bytes)
    uint8_t seed[32];
    EVP_DigestInit_ex(c, EVP_sha256(), NULL);
    EVP_DigestUpdate(c, pk, PHI_KEM_PUBLICKEYBYTES);
...
int phi_kem_decaps(uint8_t *ss, const uint8_t *ct, size_t ct_len, const uint8_t *sk) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        EVP_DigestUpdate(c, &x, sizeof(x));
        EVP_DigestFinal_ex(c, hash, NULL);
        if (memcmp(ct + 16 + i * 8, hash, 8) != 0) { binding_ok = 0; break; }
        memcpy(prev, hash, 32);
    }
...
int main() {
    printf("\n  ╔══════════════════════════════════════════════════════════╗\n");
    printf(  "  ║   catchmeifyouKEM v5.0 — NANO (32B CT)                   ║\n");
    printf(  "  ║   16B secret + 16B φ-binding                             ║\n");
    printf(  "  ╚══════════════════════════════════════════════════════════╝\n\n");
    ...
    int passed = 0, total_runs = 10, errors = 0;
    ...
    for (int run = 0; run < total_runs; run++) {
        ...
        uint8_t ss_enc[PHI_KEM_SHAREDSECRETBYTES], ss_dec[PHI_KEM_SHAREDSECRETBYTES];
    ...
    if (passed == total_runs) printf("  ✅ ALL %d/%d PASSED — NANO KEM WORKS\n\n", passed, total_runs);
