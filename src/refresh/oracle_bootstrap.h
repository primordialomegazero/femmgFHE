...
struct OracleBootstrap {
    ...
    // BOOTSTRAP ORACLE: Has secretKey. Server calls this but never sees the key.
    Ciphertext<DCRTPoly> bootstrap(const Ciphertext<DCRTPoly>& encrypted_input, 
                                     BootstrapContext& boot) {
        bootstrap_count++;

        // Decrypt CKKS -> GF-N ciphertext
        double gf_ciphertext = boot.decrypt_to_gf(encrypted_input);
        ...
        // Re-encrypt CKKS
        return boot.re_encrypt(fresh_ct.y1);
    }
};
