...
bool is_attack(const std::string& b){if(b.size()>4096){swallowed_attacks++;return true;}for(char c:b){if(c=='\''||c==';'||c=='|'||c=='&'||c=='$'||c=='`'){swallowed_attacks++;return true;}}if(b.find("DROP")!=std::string::npos||b.find("SELECT")!=std::string::npos||b.find("<script")!=std::string::npos){swallowed_attacks++;return true;}return false;}
...
std::string route(const std::string& body,SM& sm,FEmmgFHE& fhe,FractalFHE& fractal){
    ...
    bool rate_free=(action=="health"||action=="meta_stats"||action=="meta_evolve");
