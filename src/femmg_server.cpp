...
class SM{std::map<std::string,Session> s;std::mutex m;public:void reg(const std::string& id){std::lock_guard<std::mutex> l(m);if(s.find(id)==s.end())s[id]=Session{id,{},0};}bool has(const std::string& id){std::lock_guard<std::mutex> l(m);return s.find(id)!=s.end();}void inc(const std::string& id){std::lock_guard<std::mutex> l(m);auto it=s.find(id);if(it!=s.end())it->second.reqs++;}uint64_t store(const std::string& id,const godcode::NDimCiphertext& ct){std::lock_guard<std::mutex> l(m);auto it=s.find(id);if(it==s.end())return 0;it->second.cts.push_back(ct);return it->second.cts.size()-1;}bool get(const std::string& id,uint64_t idx,godcode::NDimCiphertext& out){std::lock_guard<std::mutex> l(m);auto it=s.find(id);if(it==s.end()||idx>=it->second.cts.size())return false;out=it->second.cts[idx];return true;}uint64_t total(){std::lock_guard<std::mutex> l(m);return s.size();}};
...
std::string route(const std::string& body,SM& sm,FEmmgFHE& fhe,FractalFHE& fractal){
    ...
    bool rate_free=(action=="health"||action=="meta_stats"||action=="meta_evolve"||action=="zkp_prove"||action=="zkp_fractal"||action=="fhe_store");
    ...
    if(action=="fhe_store"){if(!sm.has(cid)){unregistered_attempts++;return ok(bh());}double encrypted=sd(sg(body,"encrypted_value"));int party=(int)sd(sg(body,"party"));sm.inc(cid);godcode::NDimCiphertext ct{};ct.coordinates[0]=encrypted;ct.expanded_dim0=encrypted;ct.party_id=party;ct.noise=40.0;uint64_t idx=sm.store(cid,ct);return ok(O({J("action","fhe_store"),I("ciphertext_index",idx),N("encrypted_dim0",encrypted),I("party",party),B("server_saw_plaintext",false),B("true_zero_knowledge",true)}));}
