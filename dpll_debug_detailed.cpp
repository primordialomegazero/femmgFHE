...
struct DebugDPLL {
    ...
    void print_assignment() {
        std::cout << "  Assignment: ";
        for (int v = 1; v <= std::min(n_vars, 10); v++) {
            std::cout << "x" << v << "=" << (assignment[v] == 1 ? "T" : assignment[v] == -1 ? "F" : "?") << " ";
        }
        if (n_vars > 10) std::cout << "...";
        std::cout << "\n";
    }
    ...
    int select_var() {
        ...
        std::cout << "  Selected: x" << best << " (score=" << std::fixed << std::setprecision(2) << best_score << ")\n";
        return best;
    }
...
int main() {
    ...
    // Generate the problematic instance
