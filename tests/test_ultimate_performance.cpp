...
int main() {
    std::cout << "=== GOLDEN ULTIMATE SOLVER ===\n\n";
    ...
    // 3-coloring
    std::cout << "--- 3-Coloring ---\n";
    for (int num_vertices = 10; num_vertices <= 200; num_vertices += 10) {
        for (int test = 0; test < 3; test++) {
            ...
            if (result.satisfiable) {
                ...
                if (num_vertices % 50 == 0) {
                    std::cout << "  " << num_vertices << " vertices: PASS ("
                             << result.time_ms << "ms)\n";
                }
            } else {
                std::cout << "  " << num_vertices << " vertices: FAIL\n";
            }
        }
    }
