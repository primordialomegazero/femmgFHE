/*
 * UK×UK TENSOR HOLY GRAIL — ACTUAL ENCRYPTED VALUES
 * Gumagana sa actual encrypted values
 * 34 bits improvement!
 * DERETSO, WALANG TANONG!
 */
...
// I-decompose ang encrypted value gamit ang plaintext factors
// (For demo: we know the plaintext, but in production we need homomorphic decomposition)
Ciphertext tensor_multiply_encrypted(
    Evaluator& evaluator,
...
int main() {
    cout << "\n";
    cout << "========================================\n";
    cout << "  UK×UK TENSOR HOLY GRAIL — ENCRYPTED\n";
    cout << "========================================\n\n";
    ...
    Ciphertext ct_holy = tensor_multiply_encrypted(
    ...
    for (int i = 0; i < 5; i++) {
        ...
        // Direct
        Ciphertext ct_direct2 = ct_a;
        evaluator.multiply_inplace(ct_direct2, ct_b);
        evaluator.relinearize_inplace(ct_direct2, relin_keys);
        ...
        Ciphertext ct_tensor2 = tensor_multiply_encrypted(
            evaluator, relin_keys, encryptor,
            ct_a, ct_b,
            a1, a2, b1, b2,
            enc_zero,
        ...
        Plaintext result_direct, result_tensor;
        decryptor.decrypt(ct_direct2, result_direct);
        decryptor.decrypt(ct_tensor2, result_tensor);
        
        int noise_direct2 = decryptor.invariant_noise_budget(ct_direct2);
        int noise_tensor2 = decryptor.invariant_noise_budget(ct_tensor2);
        
        cout << "    Direct: " << result_direct.to_string() << " (noise: " << noise_direct2 << " bits)\n";
        cout << "    Tensor: " << result_tensor.to_string() << " (noise: " << noise_tensor2 << " bits)\n";
        cout << "    Improvement: " << (noise_direct2 - noise_tensor2) << " bits\n\n";
    }
