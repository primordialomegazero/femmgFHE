...
int main() {
    std::cout << "RLWE HOMOMORPHIC NAND DEEP CHAIN\n";
    std::cout << "================================\n\n";
    ...
    auto hom_nand = [&](std::pair<NTL::ZZ_pX, NTL::ZZ_pX> a,
        ...
        // NAND = E_Lk - scaled
        return std::make_pair(E_Lk.first - scaled.first, E_Lk.second - scaled.second);
    };

    // Test
    auto E_ct0 = encrypt(NTL::to_ZZ(0));
    auto E_ct1 = encrypt(L_k);

    std::cout << "Initial decrypt:\n";
    ...
    for (int i = 0; i <= 100; i++) {
        ...
        std::cout << "  Depth " << i << ": " << dec << " (exp " << (expected ? L_k : NTL::to_ZZ(0)) << ")\n";
        current = hom_nand(current, current);
    }
    std::cout << "\nErrors: " << errors << "/11\n";
