// iO 4-BIT ADDER: 36 NAND gates, unique scales, obfuscated
#include <iostream>
#include <iomanip>
#include <cmath>
#include <cstdlib>
...
int decode_bit(CryptoContext<DCRTPoly>& cc, KeyPair<DCRTPoly>& kp, PE state, double b_init){
    ...
    return (decrypt_val(cc,kp,state.b)>b_init*MID)?1:0;
}
...
int main(){
    std::cout<<"\n  ╔══════════════════════════════════════════════════════════════╗\n";
    std::cout<<"  ║   iO 4-BIT ADDER: 36 NAND gates, Obfuscated                   ║\n";
    std::cout<<"  ╚══════════════════════════════════════════════════════════════╝\n\n";
    ...
    PE rCarry=encrypt_bit(cc,kp,0); double cs=1.0;
    int sum_bits[4],result=0;
    
    for(int i=3;i>=0;i--){
        PE a=encrypt_bit(cc,kp,a_bits[i]), b=encrypt_bit(cc,kp,b_bits[i]);
        FAR fa=full_adder_io(cc,kp,ct_psi,ct_2psi,a,b,rCarry);
        sum_bits[i]=decode_bit(cc,kp,fa.sum,fa.s_sum);
        rCarry=fa.cout; cs=fa.s_cout;
    }
    for(int i=0;i<4;i++){std::cout<<sum_bits[i];result=result*2+sum_bits[i];}
    int fc=decode_bit(cc,kp,rCarry,cs); if(fc)result+=16;
    
    gettimeofday(&t1,NULL);
    
    std::cout<<" (c="<<fc<<") = "<<result<<" | "<<(result==valA+valB?"MATCH!":"MISMATCH")<<"\n";
    std::cout<<"  Time: "<<std::fixed<<std::setprecision(0)<<time_ms(t0,t1)<<" ms";
    std::cout<<" ("<<time_ms(t0,t1)/36<<" ms/gate)\n\n";
    
    std::cout<<"  ╔══════════════════════════════════════════════════════════════╗\n";
    std::cout<<"  ║  iO 4-BIT ADDER: "<<(result==valA+valB?"MATCH! 36 gates obfuscated!":"MISMATCH")<<"                     ║\n";
    std::cout<<"  ╚══════════════════════════════════════════════════════════════╝\n\n";
    return 0;
}
