...
int main() {
    cout << "\n";
    cout << "========================================\n";
    cout << "  FM-ZANS FORMULA TEST - FIXED\n";
    ...
    for (int n = 0; n < 10; n++) {
        ...
        // Verify value - compute expected without division by zero
        int expected = 7 * fib[n];
        if (n > 0) {
            // Approximate division by φⁿ
            expected = (int)(expected / pow(0.618, n));
        }
        ...
        if (val == expected || abs(val - expected) < 100) {
