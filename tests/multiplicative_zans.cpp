/*
 * MULTIPLICATIVE ZANS (ZANS-M)
 * I-test kung gumagana ang ZANS sa multiplication
 * Combine with tensor decomposition
 */
...
int main() {
    ...
    cout << "--- TEST: ZANS-M (×Enc(1)) on multiplication ---\n\n";
    
    Plaintext plain_12("12"), plain_34("34"), plain_1("1");
    Ciphertext ct_12, ct_34, ct_1;
    ...
    cout << "  Direct + 1000 ZANS: " << noise_zans << " bits (no change)\n";

    // ZANS-M: multiply by Enc(1)
    Ciphertext ct_zans_m = ct_direct;
    for (int i = 0; i < 10; i++) {
        evaluator.multiply_inplace(ct_zans_m, ct_1);
        evaluator.relinearize_inplace(ct_zans_m, relin_keys);
    }
    int noise_zans_m = decryptor.invariant_noise_budget(ct_zans_m);
    cout << "  Direct + 10×Enc(1): " << noise_zans_m << " bits (worse)\n";
    ...
    // Tensor + ZANS-M
    Ciphertext ct_tensor_zans_m = ct_tensor;
    for (int i = 0; i < 10; i++) {
        evaluator.multiply_inplace(ct_tensor_zans_m, ct_1);
        evaluator.relinearize_inplace(ct_tensor_zans_m, relin_keys);
    }
    int noise_tensor_zans_m = decryptor.invariant_noise_budget(ct_tensor_zans_m);
    cout << "  Tensor + 10×Enc(1): " << noise_tensor_zans_m << " bits\n";

    cout << "\n========================================\n";
    cout << "  CONCLUSION\n";
    cout << "========================================\n";
    cout << "  ZANS (addition) works: " << (noise_zans == noise_direct ? "✅ No change (good)" : "❌") << "\n";
    cout << "  ZANS-M (multiplication) fails: " << (noise_zans_m > noise_direct ? "❌ Adds noise" : "✅") << "\n";
    cout << "  Tensor improves: " << (noise_tensor < noise_direct ? "✅ " + to_string(noise_direct - noise_tensor) + " bits" : "❌") << "\n";
    cout << "  Tensor + ZANS-M: " << (noise_tensor_zans_m > noise_tensor ? "❌ Adds noise" : "✅") << "\n";
    cout << "========================================\n\n";
