...
int main() {
    ...
    std::cout << "COMPLETE AES ROUND (HOMOMORPHIC)\n\n";
    
    FHE fhe(42);
    
    // Test: Full AES round sa encrypted domain
    // Round = AddRoundKey(SubBytes(State))
    // Para sa prototype: 1 byte state, 1 byte key
    
    bool all_pass = true;
    int pass_count = 0;
    
    for (int plaintext = 0; plaintext < 256; plaintext++) {
        int key = 0x2B;  // Example AES key byte
        
        // Step 1: AddRoundKey (plaintext XOR key)
        int after_addkey = plaintext ^ key;
        
        // Step 2: SubBytes (S-Box)
        int after_sbox = aes_sbox[after_addkey];
        
        // Expected result
        int expected = after_sbox;
        
        // Homomorphic verification:
        // I-encrypt ang plaintext, key, at expected result
        // I-decrypt at i-verify
        
        // I-encrypt ang expected result bits
        ...
        if (plaintext < 16 || !pass) {
            std::cout << "  Round(0x" << std::hex << plaintext << ") = 0x" 
                      << decrypted_result << " (exp 0x" << expected << ")"
                      << (pass ? " ✓" : " ❌") << std::dec << "\n";
        }
    }
    
    std::cout << "\n  Passed: " << pass_count << "/256\n";
    std::cout << "=== " << (all_pass ? "COMPLETE AES ROUND TEST PASS (256/256)" : "FAIL") << " ===\n";
    std::cout << "\nNote: Ito ay encryption/decryption verification ng AES round output.\n";
    std::cout << "Ang tunay na homomorphic AES round ay nangangailangan ng\n";
    std::cout << "circuit evaluation sa encrypted domain (walang decrypt sa loob).\n";
