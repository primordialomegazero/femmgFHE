...
int main() {
    ...
    GoldenBootstrapping::UnlimitedFHE fhe(pk, sk, 1);  // Bootstrap after every op
    ...
    NTL::ZZ_pX noise_a = a.c0 + a.c1 * sk.sk;
    long v_a = NTL::conv<long>(NTL::coeff(noise_a, 0));
    std::cout << "a: " << v_a << " (should be 0)\n";
    ...
    std::cout << "NAND(0,0): " << v_nand_ab << " (should be " << golden_plain << ")\n";
    
    // NAND(a, NAND(a,b)) = NAND(0, 1) = 1
    ...
    std::cout << "NAND(0,1): " << v_nand_a_ab << " (should be " << golden_plain << ")\n";
    
    // NAND(b, NAND(a,b)) = NAND(0, 1) = 1
    ...
    std::cout << "NAND(0,1): " << v_nand_b_ab << " (should be " << golden_plain << ")\n";
    
    // XOR = NAND(NAND(a,nand_ab), NAND(b,nand_ab)) = NAND(1,1) = 0
    ...
    std::cout << "XOR(0,0): " << v_xor << " (should be 0, less than " << threshold << ")\n";
