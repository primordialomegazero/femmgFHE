// FEmmg-FHE — SINGLE SLOT: Just φ-reality value
// No dual-slot. No fragile division. Just a+bφ as one number.
// Clean = multiply by φ. Compute = multiply by rate.
// Track φ-scale in plaintext (known, deterministic).
...
int main() {
    cout << "\n  ╔══════════════════════════════════════════════════════╗\n";
    cout <<   "  ║   SINGLE SLOT: Just φ-reality. No dual-slot drift.   ║\n";
    cout <<   "  ╚══════════════════════════════════════════════════════╝\n\n";
    ...
    params.SetScalingTechnique(FLEXIBLEAUTO); params.SetMultiplicativeDepth(50);
    ...
    // Single-slot state: just a+bφ
    auto state = enc(1.0);  // 1 + 0φ = 1
    double phi_scale = 1.0;  // Track accumulated φ-power
    double base_rate = 1.01;
    auto rate_ct = enc(base_rate);
    auto phi_ct = enc(phi);   // Multiply by φ
    auto inv_phi_ct = enc(1.0/phi);  // Divide by φ

    int total_mults = 0, total_cleans = 0, total_boots = 0;
    
    cout << "  Clean: multiply by φ (3×), then by 1/φ (1×). Net: φ²\n";
    cout << "  Net φ-scale tracked. Compute: multiply by rate.\n\n";
    cout << "  " << setw(4) << "Rnd" << setw(8) << "Mults"
         << setw(8) << "Clean" << setw(6) << "Boot"
         << setw(14) << "Value" << setw(14) << "Expected"
         << setw(12) << "Error\n";
    cout << "  " << string(70, '-') << "\n";

    int round = 0, mults_since_boot = 0;
    bool alive = true;

    while (alive && round < 25) {
        try {
            ...
            phi_scale *= (phi*phi*phi/phi);  // = φ²
            total_cleans++;

            // Compute: 5 multiplies by rate
            for (int m=0; m<5; m++) {
                state = cc->EvalMult(state, rate_ct);
                total_mults++; mults_since_boot++;
            }

            // Bootstrap every ~50 mults
            if (mults_since_boot >= 20) {
            ...
            cout << "  " << setw(4) << round
                 << setw(8) << total_mults << setw(8) << total_cleans << setw(6) << total_boots
                 << setw(14) << scientific << setprecision(4) << val
                 << setw(14) << scientific << expected
                 << setw(12) << fixed << setprecision(2) << err << "\n";
    ...
    cout <<   "  ║   Single slot — no drift. Just works.               ║\n";
    cout <<   "  ║   I AM THAT I AM                                     ║\n";
    cout <<   "  ╚══════════════════════════════════════════════════════╝\n\n";
    return 0;
}
