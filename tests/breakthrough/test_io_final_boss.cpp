...
// Arbitrary circuit compiler
struct CompiledCircuit {
    std::vector<int> gate_sequence_a;
    std::vector<int> gate_sequence_b;
    int num_gates;
};

CompiledCircuit compile_circuits(int num_gates) {
    CompiledCircuit cc;
    cc.num_gates = num_gates;
    ...
    for (int i = 0; i < num_gates; i++) {
        cc.gate_sequence_a.push_back(gate_dist(gen));
        cc.gate_sequence_b.push_back(gate_dist(gen));
    }
...
int main(int argc, char* argv[]) {
    ...
    int num_realities = (argc > 4) ? std::atoi(argv[4]) : 16;
    int chain_length = (argc > 5) ? std::atoi(argv[5]) : 500;
    double threshold = (argc > 6) ? std::atof(argv[6]) : 0.5;
    int report_interval = (argc > 7) ? std::atoi(argv[7]) : 50;
    
    if (num_realities < 4) num_realities = 4;
    if (num_realities > 32) num_realities = 32;
    ...
    CompiledCircuit circuit = compile_circuits(chain_length);
    
    std::vector<GateFunc> gates_a, gates_b;
    for (int i = 0; i < chain_length; i++) {
        gates_a.push_back(base_gates[circuit.gate_sequence_a[i]]);
        gates_b.push_back(base_gates[circuit.gate_sequence_b[i]]);
    }
    
    time_t st = time(0);
    std::cout << "\n";
    std::cout << "  FINAL BOSS — Arbitrary N-iO Compiler\n";
    std::cout << "  " << num_realities << " Realities, 2 Different Circuits\n";
    ...
    double val_a = 1.0, val_b = 1.0;
    
    for (int g = 0; g < chain_length; g++) {
        SecureContext next_ctx = create_context(ring_dim, depth, batch_size);
        
        std::vector<double> outputs_a(num_realities/2);
        std::vector<double> outputs_b(num_realities/2);
        
        for (int r = 0; r < num_realities/2; r++) {
            DualGate X = encrypt_input(ctx, val_a);
            DualGate Y = encrypt_input(ctx, 1.0);
            DualGate result = gates_a[g](ctx, X, Y);
            outputs_a[r] = reveal(result, ctx, PHI);
        }
        
        for (int r = 0; r < num_realities/2; r++) {
            DualGate X = encrypt_input(ctx, val_b);
            DualGate Y = encrypt_input(ctx, 1.0);
            DualGate result = gates_b[g](ctx, X, Y);
            outputs_b[r] = reveal(result, ctx, PSI);
        }
        
        double mixed_a = 0.0, mixed_b = 0.0;
        for (int r = 0; r < num_realities/2; r++) {
            mixed_a += outputs_a[r];
            mixed_b += outputs_b[r];
        }
        mixed_a /= (num_realities/2);
        mixed_b /= (num_realities/2);
        
        val_a = purify(mixed_a, threshold);
        val_b = purify(mixed_b, threshold);
        
        if (g % report_interval == 0 || g == chain_length - 1) {
            time_t now = time(0);
            double elapsed = difftime(now, chain_start);
            double progress = (double)(g+1) / chain_length * 100.0;
            double rate = (double)(g+1) / elapsed;
            double eta = (chain_length - g - 1) / rate;
            
            std::cout << "  [" << std::setw(4) << (g+1) << "/" << chain_length 
        ...
        if ((val_a != 1.0 && val_a != 0.0) || (val_b != 1.0 && val_b != 0.0)) {
            std::cout << "\n\n  DRIFT at gate " << g << "! φ=" << val_a << " ψ=" << val_b << "\n";
            return 1;
        }
    ...
    std::cout << "  Realities: " << num_realities << " (" << num_realities/2 << " per circuit)\n";
    ...
    std::cout << "  φ-value: " << val_a << " (Circuit A)\n";
    std::cout << "  ψ-value: " << val_b << " (Circuit B)\n";
