// OPENFHE DEEP + ARBITRARY CIRCUITS — FIXED v2
#include <NTL/ZZ.h>
#include "openfhe.h"
#include <iostream>
#include <vector>
...
int main() {
    std::cout << "========================================\n";
    std::cout << "  OPENFHE DEEP + ARBITRARY CIRCUITS\n";
    std::cout << "  BFV + φ-Structure\n";
    std::cout << "========================================\n\n";

    // Setup BFV — tamang parameters
    CCParams<CryptoContextBFVRNS> parameters;
    parameters.SetPlaintextModulus(65537);
    parameters.SetMultiplicativeDepth(30);
    parameters.SetRingDim(65536);  // 2^16
    ...
    std::vector<int64_t> vec_phi_k(65536, 0);
    vec_phi_k[0] = NTL::to_long(phi_k_plain);

    std::vector<int64_t> vec_inv_phi_k(65536, 0);
    vec_inv_phi_k[0] = NTL::to_long(inv_phi_k_plain);

    std::vector<int64_t> vec_zero(65536, 0);
    std::vector<int64_t> vec_one(65536, 0);
    ...
    auto decrypt_to_bool = [&](Ciphertext<DCRTPoly> ct) {
        ...
        return (val == NTL::to_long(phi_k_plain)) ? 1 : 0;
    };
    ...
    std::cout << "\n  Deep chain result: " << errors << "/30 errors ("
              << (100.0 * (30 - errors) / 30) << "%)\n\n";

    // ============================================
    // TEST 2: ARBITRARY CIRCUITS
    // ============================================
    std::cout << "TEST 2: ARBITRARY CIRCUITS\n";
    std::cout << "============================\n\n";
    ...
    // XOR
    std::cout << "  XOR Gate:\n";
    std::cout << "  A B | Expected | Got | OK?\n";
    std::cout << "  ----|----------|-----|-----\n";

    for (auto a : inputs) {
        for (auto b : inputs) {
            auto result = homomorphic_xor(a, b);
            int got = decrypt_to_bool(result);
            int a_val = decrypt_to_bool(a);
            int b_val = decrypt_to_bool(b);
            int expected = a_val ^ b_val;

            std::cout << "  " << a_val << " " << b_val << " | " << expected
                      << "        | " << got
                      << "   | " << (expected == got ? "YES" : "NO") << "\n";
        }
    }
    ...
    for (auto a : inputs) {
        for (auto b : inputs) {
            auto result = homomorphic_and(a, b);
            int got = decrypt_to_bool(result);
            int a_val = decrypt_to_bool(a);
            int b_val = decrypt_to_bool(b);
            int expected = a_val & b_val;
    ...
    for (auto a : inputs) {
        for (auto b : inputs) {
            auto result = homomorphic_or(a, b);
            int got = decrypt_to_bool(result);
            int a_val = decrypt_to_bool(a);
            int b_val = decrypt_to_bool(b);
            int expected = a_val | b_val;
    ...
    // TEST 3: FULL ADDER
    // ============================================
    std::cout << "TEST 3: FULL ADDER (1-bit)\n";
    std::cout << "=============================\n\n";

    std::cout << "  A B Cin | Sum | Cout | OK?\n";
    std::cout << "  --------|-----|------|-----\n";
