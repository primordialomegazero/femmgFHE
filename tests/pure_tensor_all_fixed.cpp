 ...
 * I-fix ang lahat ng ratios including 11-15
 */
...
// Pure Tensor with universal adaptive decomposition
Ciphertext pure_tensor_universal(
    Evaluator& evaluator,
    ...
    // Universal adaptive decomposition
    int a1, a2, b1, b2;
    if (split_ratio > 10) {
        // Para sa malalaking ratios, gumamit ng 2+2 decomposition
        a1 = a / 2;
        a2 = a - a1;
        b1 = b / 2;
        b2 = b - b1;
    } else if (split_ratio == 4) {
...
int main() {
    ...
    vector<int> ratios;
    for (int i = 2; i <= 30; i++) ratios.push_back(i);
    ...
    for (int ratio : ratios) {
        try {
            Ciphertext ct_tensor = pure_tensor_universal(
