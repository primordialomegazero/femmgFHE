// φ + MODREDUCE + EVALBOOTSTRAP — FULL COMBO
// Ang kumpletong bootstrapping system
...
int main() {
    std::cout << "========================================\n";
    std::cout << "  FULL COMBO: φ + MODREDUCE + BOOTSTRAP\n";
    std::cout << "========================================\n\n";

    // Setup na may FHE capability
    ...
    std::cout << "FULL BOOTSTRAP TEST (40 gates):\n";
    std::cout << "================================\n\n";
    ...
    for (int gate = 0; gate < total_gates; gate++) {
        // Every 10 gates: full refresh cycle
        if (gate > 0 && gate % 10 == 0) {
            // 1. φ-refresh
            if (scale_idx == 0) {
            ...
            // 2. ModReduce para sa noise reduction
            current = cc->ModReduce(current);

            // 3. EvalBootstrap para sa level reset
            try {
        ...
        if (gate < 3 || gate % 10 == 9 || gate >= total_gates - 3) {
