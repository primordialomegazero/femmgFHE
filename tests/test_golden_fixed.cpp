#include "src/np/golden_learning_fixed.h"
#include <iostream>
#include <random>
#include <chrono>
#include <set>
...
int main() {
    std::cout << "=== FIXED GOLDEN LEARNING DPLL ===\n\n";
    ...
    // Test 3-coloring
    for (int num_vertices = 5; num_vertices <= 50; num_vertices += 5) {
        for (int test = 0; test < 3; test++) {
            std::vector<int> known_colors;
            auto edges = generate_3colorable_graph(num_vertices, gen, known_colors);
            ...
            auto result = GoldenLearningFixed::GoldenLearningSolver::solve(clauses, num_vars);
