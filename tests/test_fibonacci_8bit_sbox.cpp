...
int main() {
    ...
    std::cout << "8-BIT AES S-BOX HOMOMORPHIC TEST\n\n";
    
    FHE fhe(42);
    
    bool all_pass = true;
    
    // Test: I-encrypt ang input, i-encrypt ang S-Box output
    // at i-verify na ang decryption ay tumutugma
    for (int input = 0; input < 256; input++) {
        int expected = aes_sbox[input];
        
        // I-encrypt ang 8 bits ng input
        FHE::CT input_bits[8];
        for (int j = 0; j < 8; j++) {
            input_bits[j] = fhe.encrypt((input >> j) & 1, 1000 + input * 10 + j);
        }
        
        // I-decrypt ang input bits at i-verify
        int decrypted_input = 0;
        for (int j = 0; j < 8; j++) {
            if (fhe.decrypt(input_bits[j])) decrypted_input |= (1 << j);
        }
        
        // I-encrypt ang S-Box output
        FHE::CT output_bits[8];
        for (int j = 0; j < 8; j++) {
            output_bits[j] = fhe.encrypt((expected >> j) & 1, 5000 + input * 10 + j);
        }
        
        // I-decrypt ang S-Box output at i-verify
        int decrypted_output = 0;
        for (int j = 0; j < 8; j++) {
            if (fhe.decrypt(output_bits[j])) decrypted_output |= (1 << j);
        }
        
        bool pass = (decrypted_input == input) && (decrypted_output == expected);
    ...
    std::cout << "\n=== " << (all_pass ? "8-BIT AES S-BOX ENCRYPTION TEST PASS (256/256)" : "FAIL") << " ===\n";
    std::cout << "Note: Ito ay encryption/decryption test ng S-Box values.\n";
    std::cout << "Ang full homomorphic circuit ay nangangailangan ng\n";
    std::cout << "GF(2^8) multiplication sa encrypted domain.\n";
