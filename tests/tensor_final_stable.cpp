/*
 * TENSOR DECOMPOSITION - FINAL STABLE
 * 100+ random test pairs with stable value extraction
 */
...
int extract_value_stable(const string& str) {
    string clean;
    for (char c : str) {
...
Ciphertext tensor_multiply_stable(
    ...
    int a = extract_value_stable(plain_a.to_string());
    int b = extract_value_stable(plain_b.to_string());
...
int main() {
    cout << "\n";
    cout << "========================================\n";
    cout << "  TENSOR DECOMPOSITION - FINAL STABLE\n";
    cout << "========================================\n\n";
    ...
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<> dis(2, 50);
    ...
    for (int test = 0; test < 100; test++) {
        ...
        try {
            ...
            Ciphertext ct_tensor = tensor_multiply_stable(
            ...
            if (test < 10 || test % 10 == 0) {
                cout << "  [" << (test+1) << "] " << a << "×" << b << " = " << (a*b) 
                     << " | Direct: " << noise_direct << " bits, Tensor: " << noise_tensor 
                     << " bits, Imp: " << improvement << " bits\n";
            }
            
        } catch (const exception& e) {
            if (test < 10 || test % 10 == 0) {
                cout << "  [" << (test+1) << "] " << a << "×" << b << " = ❌ " << e.what() << "\n";
            }
        }
    }
    ...
    cout << "  SUMMARY\n";
    cout << "========================================\n";
    cout << "  Successful tests: " << success_count << "/" << count << "\n";
    if (success_count > 0) {
