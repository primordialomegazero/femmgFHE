// BINFHE COMPARATOR — FIXED
// Tamang cascade logic
...
int main() {
    std::cout << "========================================\n";
    std::cout << "  BINFHE COMPARATOR — FIXED\n";
    std::cout << "========================================\n\n";
    ...
    // Tamang comparator: MSB-first comparison
    auto comparator = [&](auto a_bits, auto b_bits) {
        auto result = cc.Encrypt(sk, 0);    // FALSE initially
        auto prev_equal = cc.Encrypt(sk, 1); // Assume equal muna

        for (int i = 7; i >= 0; i--) {
            auto diff = XOR(a_bits[i], b_bits[i]);
            auto not_diff = NOT(diff);
            auto not_b = NOT(b_bits[i]);
            auto gt = AND(a_bits[i], not_b);
            
            // result |= (prev_equal AND gt)
            auto term = AND(prev_equal, gt);
            result = OR(result, term);
            
            // prev_equal = prev_equal AND NOT(diff)
            prev_equal = AND(prev_equal, not_diff);
        }
    ...
    std::cout << "TESTING 32 RANDOM SAMPLES...\n";
    std::cout << "==============================\n\n";
    ...
    std::cout << "\n========================================\n";
    std::cout << "  RESULT:\n";
    std::cout << "  Errors: " << errors << "/" << total_tests << "\n";
