#include "src/np/golden_erase_v2.h"
#include <iostream>
#include <random>
#include <chrono>

int main() {
    std::cout << "=== GOLDEN ERASE V2 (Look-Ahead) SAT ===\n\n";
    ...
    for (int num_vars : {10, 50, 100, 200, 300, 500, 1000, 2000, 5000, 10000, 20000, 50000}) {
        ...
        auto result = GoldenEraseV2::GoldenEraseSolverV2::solve(clauses, num_vars);
        auto end = std::chrono::high_resolution_clock::now();
        auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();

        std::cout << num_vars << " vars: " << (result.satisfiable ? "SAT" : "UNSAT")
                 << " (steps=" << result.steps
                 << ", erased=" << result.erased_clauses
                 << ", time=" << ms << "ms)\n";
