#include "src/np/golden_fib_fusion.h"
#include <iostream>
#include <random>
#include <chrono>

int main() {
    std::cout << "=== GOLDEN-FIBONACCI FUSION SAT ===\n\n";
    ...
    for (int num_vars : {10, 50, 100, 200, 300, 500, 1000, 2000, 5000, 10000, 20000, 50000}) {
        ...
        auto result = GoldenFibFusion::GoldenFibFusionSolver::solve(clauses, num_vars);
        auto end = std::chrono::high_resolution_clock::now();
        auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();

        std::cout << num_vars << " vars: " << (result.satisfiable ? "SAT" : "UNSAT")
                 << " (steps=" << result.steps
                 << ", fib_steps=" << result.fib_steps
                 << ", time=" << ms << "ms)\n";
