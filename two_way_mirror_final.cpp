...
struct TwoWayMirror {
    ...
    TwoWayMirror(int n, const std::vector<std::vector<int>>& cls)
    ...
    void two_way_mirror() {
        for (int i = 0; i < n_vars; i++) {
            ...
            observer[i] = obs * PHI + obd * PSI;
            observed[i] = obs * PSI + obd * PHI;
...
int main() {
    ...
    int correct = 0;
    for (auto& t : tests) {
        TwoWayMirror detector(t.vars, t.clauses);
