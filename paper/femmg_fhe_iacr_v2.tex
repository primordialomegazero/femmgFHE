...
We provide empirical validation through spectral analysis of the gaps between consecutive Riemann zeta zeros (200 high-precision samples from~\cite{odlyzko,lmfdb}). A high-resolution frequency sweep reveals that $\varphi^{-1}$ matches the dominant spectral peak at $99.77\%$ of maximum power density. This connection to prime number distribution is noted as empirical motivation but is not essential to the cryptographic construction.
...
For any probabilistic polynomial-time (PPT) adversary $\mathcal{A}$ observing a sequence of $t$ consecutive outputs of the 7D CML, the advantage in predicting the $(t+1)$-th output is bounded by:
\begin{equation}
    \text{Adv}^{\text{CTU-classical}}_{\mathcal{A}} \leq O(2^{-t \cdot \lambda_{\min}})
\end{equation}
where $\lambda_{\min} \approx 0.48$ is the minimum Lyapunov exponent. For quantum adversaries, Grover's algorithm provides at most quadratic speedup:
\begin{equation}
    \text{Adv}^{\text{CTU-quantum}}_{\mathcal{A}} \leq O(2^{-t \cdot \lambda_{\min}/2})
\end{equation}
\end{assumption}
