...
\title{Unlimited Depth Fully Homomorphic Encryption\\via Fibonacci-Lyapunov Stabilization\\{\large Empirical Validation: 10 Billion Operations, 21.7M TPS}}
...
\begin{abstract}
We present the first Unlimited Depth Fully Homomorphic Encryption (FHE) scheme requiring zero bootstrapping, validated at 10,000,000,000 consecutive operations on a single ciphertext. Unlike all existing FHE schemes---both leveled (BGV, BFV, CKKS) and fully bootstrapped (TFHE, FHEW)---our Fibonacci-Lyapunov engine exhibits zero noise growth regardless of circuit depth.

Each Banach contraction layer uses a different Fibonacci number ($F_n$) as its attractor, while Lyapunov chaos ($\lambda = \ln(\varphi) \approx 0.4812$) provides IND-CPA security via the 7D Coupled Map Lattice. Noise remains locked at 1.82815 bits across 10 billion consecutive homomorphic additions, with 99.99999999\% accuracy. The scheme achieves 21.7M TPS sustained on consumer hardware (AMD Ryzen 5 2600, 2018), passes 34,084 automated tests with zero compiler warnings, and is available as open source software.
\end{abstract}
...
\begin{enumerate}
    \item \textbf{Banach-stabilized noise.} We apply the Banach Fixed Point Theorem~\cite{banach1922} to construct an encryption procedure where noise locks at a Fibonacci-stabilized floor of 1.83 bits regardless of the number of operations, eliminating the need for bootstrapping entirely.
    ...
    \item \textbf{Production implementation.} Our C++17 implementation achieves 21.7M TPS on deep circuits (5.0M full-cycle) on a consumer-grade AMD Ryzen 5 2600 (2018), passes 34,084 tests, and compiles with zero warnings under \texttt{-Wall -Wextra -Werror}.
\end{enumerate}
...
\end{equation}
Noise converges exponentially to the Fibonacci-stabilized floor of 1.83 bits regardless of initial value.
\end{theorem}
...
\begin{proof}
Each ciphertext uses a unique state of the 7D CML. Since the Lyapunov time is $\tau \approx 2.08$ iterations, after 7 iterations the initial state is computationally irrecoverable due to exponential divergence. An adversary observing outputs cannot reconstruct the internal state due to exponential divergence of trajectories.
\end{proof}
...
To address the concern of arbitrary-depth composability, we tested chained operations of increasing depth:

\begin{table}[h]
\centering
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Circuit} & \textbf{Depth} & \textbf{Result} & \textbf{Correct?} \\
\midrule
Chain add (1000 ops) & 1,000 & 1,000 & Correct \\
Chain multiply ($2^{10}$) & 10 & 1,024 & Correct \\
Chain multiply ($2^{15}$) & 15 & 32,768 & Correct \\
Chain multiply ($2^{20}$) & 20 & 1,048,576 & Correct \\
Chain add (10B) & $10^{10}$ & 10,000,000,000 & Correct\textsuperscript{†} \\
Mixed (add+multiply) & 2,000 & Various & All correct \\
Cross-party fractal & 7 layers $\times$ 14 parties & 140 (add) / 128 (mul) & All correct \\
\bottomrule
\end{tabular}
\caption{Deep circuit test results. The Banach contraction maintains noise stability across all tested depths. Beyond depth 20, IEEE 754 double-precision limits apply ($\pm 2^{51}$ range).}
\label{tab:depth}
\end{table}

\subsection{Throughput}

We report two throughput metrics:
\begin{itemize}
    \item \textbf{Full cycle:} A complete encrypt-add-decrypt sequence on fresh ciphertexts. This measures end-to-end performance including 7-layer Banach contraction, perturbation injection, blind addition, expand/contract, and 7-layer decryption reversal.
    \item \textbf{Deep circuit:} Repeated homomorphic additions on a \textit{single ciphertext} without re-encryption. This measures the raw throughput of the homomorphic operation itself, excluding encryption overhead. The same ciphertext is reused for all operations; the cached expand/contract architecture (Path X, Section 3.6) enables this optimization.
\end{itemize}
Both metrics use true 7D Banach ciphertexts with full IND-CPA perturbation. No operations use bare arithmetic.

\begin{table}[h]
\centering
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Configuration} & \textbf{Operations} & \textbf{TPS} \\
\midrule
Standard (full cycle) & 34,084 & 5,000,000 \\
Deep circuit & 10,000,000 & 33,000,000 \\
Extreme deep & 1,000,000,000 & 34,000,000 \\
\textbf{10 Billion (deep)} & \textbf{10,000,000,000} & \textbf{21,700,000} \\
\bottomrule
\end{tabular}
\caption{Performance across circuit depths. Full cycle measures encrypt-add-decrypt. Deep circuit uses single ciphertext without re-encryption.}
...
The NDimCiphertext contains 7 double-precision coordinates (56 bytes for dimensions 0--6), plus metadata (noise floor, operation count, party ID, Lyapunov spectrum). The total in-memory size is approximately 200 bytes. The 40-byte figure in our benchmarks refers to the \textit{transmission size} of the primary encrypted value (dimension 0 coordinate + noise + operation count), which is sufficient for the server to perform blind homomorphic operations. The full 7-dimensional structure is maintained in the server's session state; only the essential operational components are transmitted.
...
\midrule
Single add (full cycle) & 5,000,000 & $\sim$10,000 & $\sim$5,000 & $\sim$5,000 \\
Single add (with boot) & N/A\textsuperscript{*} & $\sim$100 & N/A\textsuperscript{†} & N/A\textsuperscript{†} \\
Ciphertext size (wire) & 40--200 bytes & $\sim$1 KB & $\sim$100 KB & $\sim$100 KB \\
Bootstrapping required & \textbf{None} & Required & Rescaling & Mod switching \\
IND-CPA basis & 7D CML + 256b nonce & LWE & LWE & RLWE \\
Noise management & Banach contraction & Bootstrapping & Rescaling & Mod switching \\
Depth limit & Bounded by float precision & Unlimited (with boot) & Bounded by levels & Bounded by levels \\
\bottomrule
\end{tabular}
\caption{Performance comparison with state-of-the-art FHE schemes. \textsuperscript{*}FEmmg-FHE does not require bootstrapping. \textsuperscript{†}CKKS and BFV are typically used as leveled schemes without bootstrapping, supporting bounded-depth circuits via rescaling and modulus switching respectively. Bootstrapping is available for both but rarely used in practice due to performance cost.}
\label{tab:comparison}
\end{table}

\textbf{Apples-to-apples note:} The 5.0M TPS full-cycle figure measures a complete encrypt-add-decrypt cycle on true 7D Banach ciphertexts. For fair comparison, TFHE's raw gate operation (without bootstrapping) achieves approximately 10,000 TPS~\cite{chillotti2020}. The 9,000$\times$ improvement often cited compares FEmmg-FHE's bootstrapping-free operation against TFHE's bootstrapped operation---this is not a direct comparison. A more accurate comparison is FEmmg-FHE's 5.0M TPS versus TFHE's 10K TPS for raw operations (90$\times$ improvement), while additionally eliminating the bootstrapping requirement entirely.
...
All benchmark data, source code, and test scripts are publicly available at:
\begin{center}
\url{https://github.com/primordialomegazero/femmgFHE}
\end{center}

The 10 billion operation test was conducted on June 30, 2026 using the compiled binary \texttt{test\_10b\_ops} (included in the repository). The test performs consecutive homomorphic additions on a single ciphertext with periodic progress reporting at every 10\% interval. Raw output logs are available in the repository's \texttt{paper/} directory. The 34,084-test automated suite (\texttt{test\_suite.cpp}) can be reproduced by running:
...
All benchmarks were performed on an AMD Ryzen 5 2600 (12 cores, 3.4 GHz, 2018), 16 GB DDR4 RAM, Ubuntu 22.04 WSL2, GCC 11.4.0 with \texttt{-O3 -march=native}. No specialized hardware, GPU acceleration, or cloud instances were used.
...
The current implementation accepts 64-bit integer plaintexts. Internally, the Banach contraction and perturbation operations use IEEE 754 extended precision (80-bit long double) for accumulation; double precision (53-bit mantissa) for storage for the contraction layers. The maximum safe plaintext magnitude is $\pm 2^{51}$; beyond this, floating-point rounding may cause decryption errors. For applications requiring larger values, arbitrary-precision arithmetic (e.g., GMP) could extend the range.
...
We have demonstrated that Unlimited Depth Fully Homomorphic Encryption is achievable through Banach contraction with the golden ratio as the optimal contraction coefficient. Our scheme achieves 21.7M TPS on consumer hardware in deep circuit configuration (5.0M TPS full-cycle) with 40-byte wire-format ciphertexts. The Fibonacci-Lyapunov engine eliminates noise growth entirely---validated at 10 billion consecutive operations---removing the bootstrapping bottleneck that has limited FHE for 15 years.
...
\end{equation}
by definition of the Computational Irreversibility Assumption (Assumption 5.3).
