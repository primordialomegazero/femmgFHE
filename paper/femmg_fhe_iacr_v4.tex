...
\textbf{Apples-to-apples note:} The 900K TPS figure measures a complete encrypt-add-decrypt cycle on true 7D Banach ciphertexts. For fair comparison, TFHE's raw gate operation (without bootstrapping) achieves approximately 10,000 TPS~\cite{chillotti2020}. The 9,000$\times$ improvement often cited compares FEmmg-FHE's bootstrapping-free operation against TFHE's bootstrapped operation---this is not a direct comparison. A more accurate comparison is FEmmg-FHE's 900K TPS versus TFHE's 10K TPS for raw operations (90$\times$ improvement), while additionally eliminating the bootstrapping requirement entirely.
...
We have demonstrated that bootstrapping-free Fully Homomorphic Encryption is achievable through Banach contraction with the golden ratio as the optimal contraction coefficient. Our scheme achieves 21.7M TPS on consumer hardware in deep circuit configuration (5.0M TPS full-cycle) with 40-byte wire-format ciphertexts. The Fibonacci-Lyapunov engine eliminates noise growth entirely---validated at 10 billion consecutive operations---removing the bootstrapping bottleneck that has limited FHE for 15 years while eliminating the bootstrapping bottleneck entirely.
