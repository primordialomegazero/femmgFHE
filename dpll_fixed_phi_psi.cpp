...
struct FixedPhiPsiDPLL {
    ...
    FixedPhiPsiDPLL(int n, const std::vector<std::vector<int>>& cls) 
        ...
        conflict_count.resize(n_vars + 1, 0);
        ...
        while (changed) {
            ...
            for (auto& cl : clauses) {
                ...
                if (unassigned == 0 && false_count == (int)cl.size()) {
                    stats.conflicts++;
                    // Update conflict scores
                    for (int lit : cl) {
                        int v = abs(lit);
                        conflict_count[v]++;
                        if (lit > 0) phi_score[v] *= 0.9; // Penalize
                ...
                if (unassigned == 1 && false_count == (int)cl.size() - 1) {
                    ...
                    // Update scores based on propagation
                    if (val == 1) {
                        phi_score[v] *= 1.2;
                        psi_score[v] *= 0.8;
                    } else {
    ...
    int select_var() {
        ...
        for (int v = 1; v <= n_vars; v++) {
            ...
            // Calculate dynamic φ-ψ score
            double phi = phi_score[v];
            double psi = psi_score[v];
            double conflict = conflict_count[v];
            
            // Heuristic: prefer variables with balanced φ-ψ and high conflict
            double score = 0;
            ...
            // Conflict contribution (VSIDS-like)
            score += conflict * 0.5;
            
            // Randomization factor (for diversity)
            score += ((double)rand() / RAND_MAX) * 0.1;
    ...
    bool solve() {
        ...
        std::fill(conflict_count.begin(), conflict_count.end(), 0);
        trail.clear();
        stats = Stats();
...
int main() {
    std::cout << "\n";
    std::cout << "╔═══════════════════════════════════════════════════════════════════════╗\n";
    std::cout << "║  🏆 FIXED φ-ψ DPLL - Dynamic Score Updates                         ║\n";
    ...
    int correct = 0;
    for (auto& t : tests) {
        FixedPhiPsiDPLL solver(t.vars, t.clauses);
    ...
    if (acc >= 100.0) {
        ...
        std::cout << "📈 " << std::fixed << std::setprecision(2) << acc << "% - Fixed dynamic scoring!\n";
    }
